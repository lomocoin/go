...
func build(pkg, dest, version, buildOS, buildArch string) {
	...
	timeFlag := fmt.Sprintf("-X github.com/lomocoin/stellar-go/support/app.buildTime=%s", buildTime)
	versionFlag := fmt.Sprintf("-X github.com/lomocoin/stellar-go/support/app.version=%s", version)
